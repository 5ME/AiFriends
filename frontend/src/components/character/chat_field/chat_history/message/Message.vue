  ...
  <div v-if="message.content" :class="showHeader ? 'mt-3' : 'mt-1'">
    ...
    <div class="group flex items-end gap-1.5"
      ...
      <span v-if="message.time && message.role === 'user'"
            class="opacity-0 group-hover:opacity-100 transition-opacity chat-text-2 text-[11px] shrink-0 pb-0.5">
    ...
    <div v-if="message.role === 'ai' && message.citations?.length"
      ...
      <button v-for="c in message.citations" :key="c.index"
              type="button"
              class="flex items-center gap-1 chat-float chat-float-hover chat-focus rounded-full px-2.5 py-1 text-xs
        ...
        <span class="shrink-0 chat-text-2">第{{ c.chunk_index + 1 }}段</span>
      </button>
    </div>
  </div>
</template>
...
.msg-markdown :deep(code) { background: var(--cbg-code); border-radius: 4px; padding: 0.1em 0.35em; font-size: 0.85em; }
.msg-markdown :deep(pre) { position: relative; background: var(--cbg-code-block); border-radius: 8px; padding: 0.6em 0.8em; margin: 0.5em 0; overflow-x: auto; }
.msg-markdown :deep(pre code) { background: transparent; padding: 0; }
.msg-markdown :deep(blockquote) { border-left: 3px solid var(--cbg-text-2); padding-left: 0.7em; margin: 0.4em 0; color: var(--cbg-text-2); }
.msg-markdown :deep(a) { color: var(--cbg-link); text-decoration: underline; word-break: break-all; }
...
.msg-markdown :deep(pre .code-copy-btn:hover) { background: var(--cbg-code-block-hover); }
</style>
