...
<template>
  <form @submit.prevent="handleSend"
    ...
    <textarea v-show="isTextMode"
              class="chat-surface-2 flex-1 min-w-0 resize-none text-base chat-text rounded-xl
                      px-3 py-2.5 leading-6 outline-none
                      placeholder:text-[var(--cbg-text-3)]"
              placeholder="文本输入"
              aria-label="消息输入"
    ...
    <button v-if="streaming"
            type="button"
            class="chat-icon-btn-active chat-focus w-12 h-12 shrink-0 rounded-full flex items-center justify-center cursor-pointer
                   tooltip tooltip-top"
            aria-label="停止生成"
            data-tip="停止"
            @click="stopGenerate">
  ...
  <div v-if="micErrorKind && isTextMode" class="shrink-0 px-4 pb-2 flex items-center gap-3">
    <p class="chat-danger text-xs flex-1"
