...
改动先定档 S/M/L；M 档以上每个阶段开始时显式报名 `Using [skill] to [purpose]`。机制层用已安装的 superpowers skills（`C:\Users\YGQ\.dsh\skills\`）：`brainstorming`（澄清需求）→ `writing-plans`（出方案）→ `using-git-worktrees` + `test-driven-development`（实施）→ `requesting-code-review`（门 3 独立 subagent 评审）→ `verification-before-completion`（门 4 证据）→ `finishing-a-development-branch`（收尾）。
...
python -m pytest web/tests/ -v         # Run all backend tests (221 tests)
...
- Tests in `web/tests/`, run with `python -m pytest web/tests/ -v` (221 tests)
...
`.github/workflows/test.yml` — 221 tests auto-run on push/PR to master:
