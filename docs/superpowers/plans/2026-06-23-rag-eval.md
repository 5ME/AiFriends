...
## 变更记录（2026-06-24 端到端验证后）

与 spec §八同步的变更，关键计划修正：

- **Task 3/4 的下载源**：`modelscope.msdatasets.MsDataset` → `datasets.load_dataset`。Task 3 新增 qrels 预加载步骤获取 `relevant_ids` → streaming 过滤 corpus。
- **DATA_DIR 移除**：HF datasets 使用自有缓存（`~/.cache/huggingface/`），不再需要 `backend/rag_eval_data/`。
- **依赖**：`requirements-dev.txt` 只保留 `datasets`（pandas/filelock 为传递依赖）。
- **Test 覆盖**：`retrieve_chunks` 的 `json.loads` metadata 解析属于 cursor 直接查询的特殊行为（mock 不触发），无新增单测——该路径仅在端到端真实 DB 连接下验证。
