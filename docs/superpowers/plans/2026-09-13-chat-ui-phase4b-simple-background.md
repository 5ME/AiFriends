...
> **Step 0（先做）· 计划自洽性核对**：本计划的规模已使它可能内部不一致（六轮评审 + 自查共抓到 11 处：文件清单漏项、token 名不一致、表格行截断、Step 编号断裂、计数与实测不符）。**实施前先跑一次下面的核对**，任何一项不通过都先修计划再动手：
>
> ```bash
> # ① Step 编号连续性（各 Task 内应为 1..n）
> # ② 表格行完整性（以 | 开头但未以 | 结尾 = 截断）
> # ③ 计划声明的 token 是否都在设计表里（反向也要查）
> # ④ 文件清单（"文件结构"表）是否覆盖所有 Create/Modify
> # ⑤ 测试计数：`it(` 实测数 vs 文档声称数
> ```
> 判据与命令细节见本 PR 提交 `f1a75fb` 的 commit message（自查脚本的四个判据）。
