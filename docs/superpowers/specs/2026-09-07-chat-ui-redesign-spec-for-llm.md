...
- **C2 创建者非设计师**：背景图内容不可预知（全白/全黑/高饱和/低清均可能），文字可读性不得依赖图片本身，必须落在**确定性深色承托**上（蒙层或文字描边；2026-09-11 裁决 P4-D6：描边色彩固定、与图片内容完全无关，其确定性不弱于蒙层）。
...
│  └─ useBackgroundAdaptive.js（composable：亮度/主色采样 + 蒙层/accent 计算）（2026-09-11：本期不实施，见文末变更登记）
```
...
| `--overlay-k` | 0.6~1.5（亮度自适应系数，§6.5；实际渐变停点 = 0.25K~0.60K）（2026-09-11：本期不实施，见文末变更登记） | 窗口/舞台蒙层浓度 |
...
| `--bubble-user` | JS 解析结果变量 `--user-bubble-bg`：`resolveUserBubble(accent)` 在 mix 70%→60%→50% 三档中取首个白字对比度 ≥4.5:1 的档位，全部不达标则回退 `#10b981`@70%（≈4.8，理论兜底——accentFallback 已保证 L∈[0.15,0.85]，50% 档数学上恒达标，该分支防御性保留，R6）；CSS 直接引用该变量（2026-09-11：本期不实施——accent 固定为 `#10b981`，现有 70% 档实测白字 4.85:1 已达标） | 己方气泡（任意 accent 下白字达标） |
...
| `--chat-bg`（简约模式） | `#f5f5f4`（浅色）/ `#1c1917`（深色，跟随系统） | 用户切换的降级背景。**2026-09-11：本期只实现深色 `#1c1917`**，不做浅色与"跟随系统"（浅色底会使一批"白字系"元素失效，登记于 W6） |

### 6.2 舞台背景（桌面端）

- 同一张角色背景图，`<img>` 铺满舞台：`object-fit: cover; transform: scale(1.1); filter: blur(24px) saturate(1.35);` 叠加 `background: rgba(0,0,0, 0.35)`。（2026-09-07 实机调优值；`--overlay-k` 自适应已取消，**0.35 定为固定值**。）
- 目的：大屏不空、氛围统一、不干扰窗口内容。移动端无舞台（窗口全屏）。

### 6.3 窗口渐变蒙层

`linear-gradient(180deg, rgba(0,0,0,0.25) 0%, rgba(0,0,0,0.60) 100%)`。**2026-09-11：K 系数与 §6.5 一并作废，蒙层固定为 0.25→0.60。**
...
> **2026-09-11 实施口径（偏离登记 W6）**：① **只做深色版**（窗口 `#1c1917` / 舞台 `#0c0a09`），不做浅色 `#f5f5f4` 与"跟随系统"——浅色底会使玻璃头部条、名字 pill、日期胶囊、引用 chips 这批白字系元素集体失效，需连带重做；② **气泡不改** daisyUI 对比色（深色底 + 白字实测 17.04:1，已远超标线，改气泡反而破坏与沉浸模式的一致性）；③ 入口改为 WindowHeader 的 **⚙ 设置弹层**（两个开关并列，LD §3.5 / Q6 落点），非下文的"月亮/减淡图标 + tooltip"。
...
| 用户气泡 | 右对齐；`--user-bubble-bg`（resolveUserBubble 解析结果，§6.1）+ 白字；圆角 16px（右上角 4px）（2026-09-11：本期不实施——沿用现有固定 `color-mix(in srgb, #10b981 70%, black)`，实测白字 4.85:1） |
...
- 所有图标按钮：`aria-label`（如"关闭对话"、"语音输入"、"停止生成"）且焦点可见（`focus-visible:ring-2 ring-white/60`）。（2026-09-11 偏离登记 W10：实现沿用项目既有的 `ring-white/40`，与 `InputField` 麦克风按钮一致，避免同页出现两套焦点环视觉；可达性不受影响。）
...
- `prefers-reduced-motion: reduce`：波形静止、三点动画降为静态、禁用 backdrop-blur 外的大幅度动画。（2026-09-11 范围说明：本项目自绘动画——骨架 shimmer 与聊天页思考中三点——已覆盖；daisyUI `loading-*` 的动画位于 `mask-image` 内嵌 SVG 的 SMIL 中，CSS 无法停止，登记为长期待办。）
...
1. **逻辑层设计**：每个组件的 state/ref/derive 清单、事件流向图（如"发送 → SSE → 追加 → 滚动"）、`useBackgroundAdaptive` 的加载时序与竞态处理（2026-09-11：该项本期不实施，见文末变更登记）。
...
| ~~W8~~ | ~~§5.3 代码块处置~~ | **作废** | 所称"spec 要求"原不存在（本文件既无 `text-shadow` 条款、亦无 §3.4），且支撑数据 3.06:1 系错口径（真值 8.78:1） |
| W9 | §2 C2 | 表述改为"确定性深色承托（蒙层或文字描边）" | 用户裁决 P4-D6 |
| W10 | §5.3 焦点环 | 登记 `ring-white/60` → `/40` 的实现偏离 | 与项目既有按钮一致 |
| W11 | 散落引用（`:138` 组件树 / `:156` ChatWindow 职责 / `:196` `--overlay-k` / `:201` `--bubble-user` / `:207` 舞台基准系数 / `:238` §6.6 / `:272` 用户气泡 / `:366` 焦点环 / `:369` reduced-motion / `:429` §14 任务边界） | 逐行加"本期不实施"标注 | v2 回写清单遗漏 |
| W12 | §6.2 舞台背景 + §6.3 窗口渐变蒙层 | `--overlay-k` 基准系数的表述改为固定值 | 同上 |
