...
| D5 | 样本存储 | **阿里云 OSS：对象级 `private` + 短期签名 URL**（桶保持现有 `public-read` 不动） | 复刻接口要求样本公网可访问，而自有服务器是自签证书（实测 `issuer=CN=8.153.201.12`），阿里云侧拉取会因证书不可信失败；走明文 HTTP 会让生物特征数据裸传；买域名配有效证书在大陆服务器需 ICP 备案，是数周流程。**实测更正（2026-09-17）**：OSS 凭据与 bucket **一直存在于 `backend/.env`**（`OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` / `OSS_BUCKET=gqyin-ai-friends` / `OSS_REGION=cn-beijing` / `OSS_ENDPOINT=oss-cn-beijing.aliyuncs.com`，凭据实测可用），且 `alibabacloud-oss-v2==1.2.5` **已在 `requirements.txt:4`** —— 所以既不是新依赖、也没有前置条件；但该桶的 ACL 实测是 **`public-read`**，与原设计假设的"私有桶"不符，因此必须**逐对象**收紧 | 上传时多一个对象级 `acl='private'` 参数。**不是新依赖、不需要新环境变量**（SDK 与凭据都已存在） |
...
4. 上传到 OSS bucket `gqyin-ai-friends`（**5 秒超时**），**对象级 ACL 显式设为 `private`** —— 桶本身是 `public-read`，所以必须逐对象收紧，不能依赖桶级设置 → 生成短期签名 URL（有效期数分钟）
...
**将来要把整个桶收紧成私有时**（可选，不阻塞 D 批）：**可行、成本很低、不需要改我们的代码**。做法：① 在控制台或 SDK 里把 `gqyin-ai-friends` 的 ACL 从 `public-read` 改成 `private`；② 我们上传样本的方式**原样不动**（对象级 `private` 与桶级 `private` 兼容，签名流程也一样）；③ 唯一的连带影响在**我们之外**：现有那 6 个手工实验样本文件会变得需要签名才能访问，而"手工贴公开 URL 做复刻"那一步要改成用 `Client.presign` 生成几分钟有效的链接（该 API 实测存在）。也就是说，切换要改的只是手工流程，不是这套实现。

**实测结论（2026-09-17，真实账号往返一次；测试用的音色与 OSS 对象均已删除）**

用本平台自己的 TTS 生成了一段 **268KB / 约 16 秒的 mp3** 样本（生成耗时 8.2s），按**与生产完全相同的顺序**跑了一遍（上传 → 签名 → `create_voice` → **立刻删样本** → 等审核）：

| 结论 | 实测值 |
|---|---|
| 对象级 `acl='private'` 在 `public-read` 桶上是否生效 | **生效**（`get_object_acl` 返回 `private`） |
| **`create_voice` 后立刻删样本是否安全** | **安全** —— 删掉之后仍从 `DEPLOYING` 变成 `OK`，说明阿里云在提交那一刻就把样本取走了；§8 对用户的"提交后立即删除"承诺成立 |
| 从提交到 `OK` 的耗时 | **约 15 秒**（第 1 次查询 `DEPLOYING`、第 2 次即 `OK`）→ Beat 每 5 分钟的节奏够用；代价是界面上"审核中"最长可能显示 5 分钟 |
| 样本格式/大小 | 本次 268KB / 约 16 秒的 mp3 **被接受** ✓（与官方"10~20 秒、≤10MB"的推荐区间一致） |
| 合成音（TTS 生成）能否作为复刻样本 | **能**（本次样本就是平台 TTS 生成的） |
| `delete_voice` 对**不存在**的 id | HTTP **400** + `code = BadRequest.ResourceNotExist` → D 批据此把"阿里云侧已不存在"识别为**删除成功** |
...
- 样本是用户的声音（个人生物特征信息）：**对象级 `private`**（桶是 `public-read`，必须逐对象收紧，不能依赖桶级设置）、短期签名 URL、复刻提交后立即删除、平台不持久化。**但第三方侧并未删除** —— 实测 `query_voice` 仍能拿到指向原始样本的签名链接（见风险 12）。因此对用户的隐私说明必须写成"样本会被提交给语音合成服务方用于复刻"，**不能写成"全链路已删除"**
...
2. **（已实测更正，原判断是错的）** 原文写的是"OSS 凭据与 bucket 尚不存在、需用户去控制台创建、**这是开工 D 批的前置条件**"。**实测（2026-09-17）**：五个键一直都在 `backend/.env`，凭据可用（`list_objects_v2` → 200），SDK `alibabacloud-oss-v2==1.2.5` 也已在 `requirements.txt`。**D 批不存在这个前置条件**。该条残余的真实风险已另立为风险 14。
...
14. **OSS 桶是 `public-read`，任何对象默认公开可读**（实测 2026-09-17）。对这套实现的直接影响：上传样本时**必须逐对象**设 `acl='private'`（D5），不能依赖桶级设置。**另一个已存在的状况**：桶里实际有 **6 个**手工实验样本（`baiyuekui.mp3` / `guanyinpusa.mp3` / `guanyv.mp3` / `hahaha.mp3` / `wukong.mp3` / **`观音菩萨.mp3`**，170~260KB；前一版风险表写"5 个"是因为当时的探针用了 `max_keys=5`，漏掉了第 6 个），它们**现在公开可下载且文件名可猜**——其中 `guanyinpusa.mp3` / `观音菩萨.mp3` 就是线上那个复刻音色的样本。定位：非生产数据、危害有限，但确实是用户语音样本且名字可枚举。**清理需用户决定**（不擅自动用别人的云端资源）；若要收紧，做法见 §6.2 末尾那段。
...
| D | 复刻接线（OSS + 阿里云 create/delete + Beat 状态刷新 + 配额）+ `clean_dirty_characters` 的 DEBUG 闸 + 删除 `voice/custom/` 未接线原型 | — | 是（**原写的"需先备妥 OSS 凭据"已更正 —— 凭据与 SDK 一直存在**） |
