...
**将来要把整个桶收紧成私有时**（可选，不阻塞 D 批）：**可行、成本很低、不需要改我们的代码**。做法：① 在控制台或 SDK 里把 `gqyin-ai-friends` 的 ACL 从 `public-read` 改成 `private`；② 我们上传样本的方式**原样不动**（对象级 `private` 与桶级 `private` 兼容，签名流程也一样）；③ 唯一的连带影响在**我们之外**：现有那 5 个手工实验样本文件会变得需要签名才能访问，而"手工贴公开 URL 做复刻"那一步要改成用 `Client.presign` 生成几分钟有效的链接（该 API 实测存在）。也就是说，切换要改的只是手工流程，不是这套实现。
...
14. **OSS 桶是 `public-read`，任何对象默认公开可读**（实测 2026-09-17）。对这套实现的直接影响：上传样本时**必须逐对象**设 `acl='private'`（D5），不能依赖桶级设置。**另一个已存在的状况**：桶里已有 5 个手工实验样本（`baiyuekui.mp3` / `guanyinpusa.mp3` / `guanyv.mp3` / `hahaha.mp3` / `wukong.mp3`，170~260KB），它们**现在公开可下载且文件名可猜**——其中 `guanyinpusa.mp3` 就是线上那个复刻音色的样本。定位：非生产数据、危害有限，但确实是用户语音样本且名字可枚举。**清理需用户决定**（不擅自动用别人的云端资源）；若要收紧，做法见 §6.2 末尾那段。
